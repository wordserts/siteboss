...
export default async function DashboardPage() {
  ...
  // Fetch real counts once tables are populated
  ...
  // Use placeholder data while empty
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <StatCard
            icon={<Users size={14} />}
            label="Workers today"
            value={displayShiftCount}
            sub="6 subcontractors"
            color="default"
          />
          <StatCard
            ...
            <div className="divide-y divide-stone-100">
              {sites.map((site, i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-3 hover:bg-stone-50 transition-colors">
                  ...
                  {site.diary === true && (
                    <span className="text-xs bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full font-medium">Diary ✓</span>
                  )}
                  {site.diary === false && (
                    <span className="text-xs bg-orange-50 text-orange-700 px-2 py-0.5 rounded-full font-medium">Diary ✗</span>
                  )}
                  {site.diary === null && (
                    <span className="text-xs bg-stone-100 text-stone-400 px-2 py-0.5 rounded-full">No diary</span>
                  )}
                </div>
              ))}
            </div>
